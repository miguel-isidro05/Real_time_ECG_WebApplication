...
def extract_mat_column_reverse(file_path, struct_name, field_name, column=None):
    # Cargar el archivo .mat
    mat_data = scipy.io.loadmat(file_path)
    ...
    # Devolver la columna especificada o todas las columnas
    if column is not None:
        return np.array(array_data[column, :]) #Cambiar si lo necesitas al reves
    
    #print(f"Tamaño de la columna seleccionada: {column.shape}")
...
def process_ecg_signal(start_segment, duration_seconds, sampling_rate, ecg_signal):
    ...
    ecg_signals, info = nk.ecg_process(segment, sampling_rate=sampling_rate)
...
def detect_anomalies(df_isolation, contamination=0.9, n_estimators=100, random_state=42):
...
def visualizar_datos(lista_datos, start_segment, duration_seconds, sampling_rate):
    ...
    end_index = min(int(start_index + (sampling_rate * duration_fs)), len(ecg_signal))  # Asegurarse de que no exceda el tamaño de la señal

    # Crear un vector de tiempo basado en la tasa de muestreo
    time_vector = np.arange(start_segment, start_segment + duration_seconds, 1 / int(sampling_rate))[:len(ecg_signal[start_index:end_index])]  # Ajustar a la longitud correcta
